...
// menu descriptions
const char _feed_desc[] PROGMEM = "FEED";
const char _shop_desc[] PROGMEM = "SHOP";
const char _play_desc[] PROGMEM = "PLAY";
const char _inventory_desc[] PROGMEM = "INVENTORY";
const char _decorate_desc[] PROGMEM = "DECORATE";
PGM_P const MENUDESCS[] PROGMEM = {
  _feed_desc, _shop_desc, _play_desc, _inventory_desc, _decorate_desc
};
...
void homeLoop() {
  ...
  if (menuPos > 0) {
    display.fillRect(44, 0, 40, 7, 0);

    for (int i=0; i < 5; i++) {
      display.drawRoundRect(24*i+7, buttonY, 18, 18, 3, 1);
      if (i == selected) {
        strcpy_P(stringbuffer, (PGM_P)pgm_read_word(&(MENUDESCS[i])));
        display.setCursor(64 - strlen(stringbuffer)*3, 0);
